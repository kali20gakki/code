...
class RoadEnhance(BaseOperator):
    ...
    def apply(self, sample, context=None):
        try:
            ...
            warnings.warn("faild to use RoadEnhance, keep sample!")
            sample = sample
        return sample
...
class MinorityEnhance(BaseOperator):
    ...
    def _apply(self, sample, context=None):
        ...
        for point in points:
            ...
            if box is not None:
                # add is_crowd
                sample['is_crowd'] = np.concatenate((sample['is_crowd'], np.array([[0]])), axis=0)
                # add gt_bbox
    ...
    def apply(self, sample, context=None):
        if random.randint(1,10) < self.prob:  # 0.6
            try:
                ...
                warnings.warn("faild to use MinorityEnhance, keep sample!")
                sample = sample
        return sample
